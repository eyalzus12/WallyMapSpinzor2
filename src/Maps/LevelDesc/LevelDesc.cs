        ...
        where T : ITexture
    {
        ...
        if (config.ShowBombsketballTargets)
        {
            ...
            if (goalblue is not null)
                canvas.DrawSwfTexture(GAMEMODE_BONES, BLUE_TARGET_SPRITE, goalblue.X + goalblue.W / 2.0, goalblue.Y + goalblue.H / 2.0, 1, trans, DrawPriorityEnum.FOREGROUND, null);
            Goal? goalred = Volumes.OfType<Goal>().Where(g => g.Team == 2).FirstOrDefault();
            if (goalred is not null)
                canvas.DrawSwfTexture(GAMEMODE_BONES, RED_TARGET_SPRITE, goalred.X + goalred.W / 2.0, goalred.Y + goalred.H / 2.0, 1, trans, DrawPriorityEnum.FOREGROUND, null);
        }
